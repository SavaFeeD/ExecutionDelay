...
export class ExecutionDelay {
  ...
  public static add(taskName: string, action: Function, delay: number = 1500) {
    let taskIndex: number = ExecutionDelay.tasks.findIndex((task) => task.name === taskName);
    ...
    function createTimer(delay: number): NodeJS.Timeout {
      return setTimeout(function () {
        const lastAction: Function = ExecutionDelay.getLastActionFromQueue(currentTask);
        lastAction();
